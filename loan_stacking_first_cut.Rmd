...
ols <- list()
ols[[1]] <- lm(def_formula2,data = loandata_comb[loandata_comb$obs<=25,])
...
stargazer(ols,type = output.type,omit = c("state"),omit.labels = c("state"),omit.stat = c("f","rsq","ser"),column.labels = c("small","medium","both"),no.space = TRUE)
...
cox <- list()
cox[[1]] <- coxph(cox_formula2,data = loandata_comb[loandata_comb$obs<=25,])
cox[[2]] <- coxph(cox_formula2,data = loandata_comb[loandata_comb$obs<=500 &loandata_comb$obs>25,])
cox[[3]] <- coxph(cox_formula2,data = loandata_comb)


stargazer(cox,type = output.type,omit = c("state"),omit.labels = c("state"),omit.stat = c("ll","logrank","max.rsq","lr","wald"),column.labels = c("small","medium","both"),no.space = TRUE,title="COXPH")
```
# Matched Regression
```{r matchedreg}
library(zoo)
...
def_formula <- as.formula("default_4months~factor(stacked)*revised_dti+fico+open_credit_lines+annual_income+credit_history_months+emp_length+factor(home_ownership)+loan_amount+loan_term+factor(state)")
def_formula2 <- as.formula("default_4months~factor(stacked)*revised_dti+fico+open_credit_lines+annual_income+credit_history_months+emp_length+factor(home_ownership)+loan_amount+loan_term+factor(state)+factor(lending_club)")
